...
import { findFutureProofModule } from "@/content/future_proof_curriculum";
import { useCompleted } from "@/lib/use-progress";
import { lessonKey } from "@/lib/progress";

export const Route = createFileRoute("/skills/$moduleSlug/")({
  loader: ({ params }) => {
    const m = findFutureProofModule(params.moduleSlug);
    if (!m) throw notFound();
    return { module: m };
  },
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.module.title} · ทักษะแห่งอนาคต 10 ปีข้างหน้า` },
